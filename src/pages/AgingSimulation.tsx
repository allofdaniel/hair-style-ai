...
import {
  ...
  getAgingInfo,
  type AgingType,
} from '../services/agingSimulation';
import { useI18n } from '../i18n/useI18n';

export default function AgingSimulation() {
  const navigate = useNavigate();
  const { t, language } = useI18n();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [userPhoto, setUserPhoto] = useState<string | null>(null);
  const [selectedCategory, setSelectedCategory] = useState<string>('younger');
  const [selectedAging, setSelectedAging] = useState<AgingType | null>(null);
  const [resultImage, setResultImage] = useState<string | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const categories = getAgingCategories();
  ...
  const handleGenerate = async () => {
    if (!userPhoto || !selectedAging) return;
    ...
    try {
      const result = await simulateAging({
        userPhoto,
        agingType: selectedAging,
      });
  ...
  const handleDownload = () => {
    ...
    link.download = `aging-${selectedAging}-${Date.now()}.png`;
    link.click();
  };

  const agingOptions = getAgingByCategory(selectedCategory);
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-10 bg-white border-b border-[#f2f4f6]">
        <div className="flex items-center justify-between h-14 px-5">
          ...
          <span className="text-[17px] font-semibold text-[#191f28]">{t('aging_simulation')}</span>
          <div className="w-10" />
        </div>
      </header>
      ...
      <main className="p-5">
        ...
        {!userPhoto ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[14px] text-[#8b95a1] mb-6 text-center">{t('aging_simulation_desc')}</p>
            <button
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div>
                <p className="text-[13px] text-[#8b95a1] mb-2 text-center">{t('current')}</p>
              ...
              <div>
                <p className="text-[13px] text-[#8b95a1] mb-2 text-center">
                  {resultImage
                    ? t('result')
                    : selectedAging
                    ? (language === 'ko' ? getAgingInfo(selectedAging).nameKo : getAgingInfo(selectedAging).name)
                    : t('select_age')}
                </p>
                <div className="aspect-[3/4] rounded-xl overflow-hidden bg-[#f2f4f6] relative">
                  {isProcessing ? (
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <p className="text-[14px] text-[#b0b8c1]">{t('select_age_preview')}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="mb-6">
              <div className="grid grid-cols-2 gap-3">
                {agingOptions.map((agingType) => {
                  const info = getAgingInfo(agingType);

                  return (
                    <button
                      key={agingType}
                      onClick={() => {
                        setSelectedAging(agingType);
                        setResultImage(null);
                      }}
                      disabled={isProcessing}
                      className={`p-4 rounded-xl transition-all text-left ${
                        selectedAging === agingType
                          ? 'bg-[#e8f3ff] border-2 border-[#3182f6]'
                          : 'bg-[#f2f4f6] border-2 border-transparent'
                      } ${isProcessing ? 'opacity-50' : ''}`}
                    >
                      <p className={`text-[14px] font-medium mb-1 ${
                        selectedAging === agingType ? 'text-[#3182f6]' : 'text-[#191f28]'
                      }`}>
                        {language === 'ko' ? info.nameKo : info.name}
                      </p>
                      <p className={`text-[11px] ${
                        selectedAging === agingType ? 'text-[#3182f6]/70' : 'text-[#8b95a1]'
                      }`}>
                        {info.description}
                      </p>
                    </button>
